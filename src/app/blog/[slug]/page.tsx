...
import { blogs, getCategory } from "@/lib/data";
...
export function generateStaticParams() {
  return blogs.map((a) => ({ slug: a.slug }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const title = `${article.title} | CodeLove Blog`;
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      type: "article",
      url: `https://codelove.in/blog/${article.slug}`,
      siteName: "CodeLove",
...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const related = blogs
    ...
    .concat(blogs.filter((a) => a.slug !== article.slug && a.category !== article.category))
    .slice(0, 3);
            ...
            <div className="container-px mx-auto max-w-5xl">
              <Reveal className="mb-8">
                <nav className="flex flex-wrap items-center gap-2 text-sm text-[var(--text-faint)]">
                  ...
                  <Link href="/blog" className="transition hover:text-[var(--text)]">Blog</Link>
                  <Icon name="chevronDown" size={14} className="-rotate-90" />
              ...
              <Reveal>
                <Link
                  href={categorySlug ? `/learn/${categorySlug}` : `/blog?category=${encodeURIComponent(article.category)}`}
          ...
          <section className="relative overflow-hidden border-t border-[var(--surface-border)] py-[50px]">
            ...
            <div className="container-px mx-auto max-w-7xl">
              <Reveal className="mb-10 flex items-end justify-between gap-4">
                ...
                <Link href="/blog" className="inline-flex shrink-0 items-center gap-1.5 text-sm font-semibold text-[var(--t-solid)] transition hover:gap-2.5">
